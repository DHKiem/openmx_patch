...
void Dos_Gaussian( char *basename, int Dos_N, double Dos_Gaussian, 
		   ...
		   int iemin, int iemax,
                   int *WhatSpecies, int *Spe_Total_CNO, int atomnum, double range );
...
void Spectra_Gaussian( int pdos_n, int *pdos_atoms, char *basename, int Dos_N, 
		       ...
		       int MaxL, int **Spe_Num_CBasis, int **Spe_Num_Relation, double range);
...
void input_main( int mode, int Kgrid[3], int atomnum, 
		 int *method, int *todo, 
		 double *gaussian, 
		 int *pdos_n, int **pdos_atoms, double *range);
  ...
  ***************************************************/
...

  {
	...
	Dos_Gaussian(  basename, Dos_N,  gaussian,  
		       ...
		       SpinP_switch, ko, ev, iemax-iemin+1,
		       emin, emax,
		       iemin,  iemax, WhatSpecies, Spe_Total_CNO, atomnum, range );
...
/*
  ...
  /*      if (mode==5){
	...
	Spectra_Gaussian( pdos_n, pdos_atoms, basename,Dos_N, gaussian, 
			  ...
			  iemin,  iemax,
			  atomnum, WhatSpecies, Spe_Total_CNO,
			  MaxL,Spe_Num_CBasis, Spe_Num_Relation , range);
        //}
                   ...
                   int *WhatSpecies, int *Spe_Total_CNO, int atomnum , double range)
{
  ...
  *****************************************************************/
  ...
  double h,s1,s2;
  double ssum[2][Dos_N];
  printf("This is the total DOS Gaussian routine. caption by DH\n");
  ...
  printf("<Dos_Gaussian> make %s\n",file_Dos); // <Dos_Gaussian> make graphene.DOS.Gaussian
  ...
  double kgrid_i = 1/(float)knum_i;
  double kgrid_j = 1/(float)knum_j;
  printf("The range near K=%f\n",range);
  //double range;
  double k_i_x,k_i_y,k_j_x,k_j_y;
  //range = 0.1443/2; // 입력 따로 받을 수 있게
  k_i_x = 1.0;
  k_i_y = 0.0;
  k_j_x = 0.5;
  k_j_y = 0.8660254037844386;
  printf("K grid i,j: %f, %f\n", kgrid_i, kgrid_j);
  
  double kx,ky;
  
  

  for (spin=0;spin<=SpinP_switch;spin++) { //0:up, 1:down
    for (i=0; i<=(knum_i-1); i++){
      for (j=0; j<=(knum_j-1); j++){
        kx = k_i_x* kgrid_i*i +  k_j_x* kgrid_j*j;
        ky = k_i_y* kgrid_i*i +  k_j_y* kgrid_j*j;
          //printf("number (i, j) : (%d, %d)\n", i, j);
          //printf("kx,ky = %f, %f\n", kx,ky);
          double klengthdiff1, klengthdiff2;
          klengthdiff1= pow(  (kx-0.5)*(kx-0.5) + (ky-0.28867513459481287)*(ky-0.28867513459481287) ,0.5 );
          klengthdiff2= pow(  (kx-1.0)*(kx-1.0) + (ky-0.5773502691896257)*(ky-0.5773502691896257) ,0.5 );
          //printf("klength = %f, %f\n", klengthdiff1,klengthdiff2);

          
	for (k=0; k<=(knum_k-1); k++){
	  for (ieg=0;ieg<neg; ieg++) { // eigenvalue number for atom


            /* generalization for spin non-collinear */
            wval = 0.0;
   	    for (GA_AN=0; GA_AN<atomnum; GA_AN++){  // number of atoms 0~N-1
  	      wanA = WhatSpecies[GA_AN];
	      tnoA = Spe_Total_CNO[wanA];
  	      for (i1=0; i1<tnoA; i1++){  // basis number for each atom
                wval += ev[spin][i][j][k][GA_AN][i1][ieg]; //orbital occupancy? Total sum = 1
                //printf("data set: (spin, i, j, k, GA_AN, i1, ieg): (%d, %d, %d, %d, %d, %d, %d)\n",spin, i, j, k, GA_AN, i1, ieg);
                //printf("wval, ev : %f, %f \n",wval, ev[spin][i][j][k][GA_AN][i1][ieg]);
	      } //i1
	    }//GA_AN

	    eg = EIGEN[spin][i][j][k][ieg] ; //energy position at k-mesh 
      //printf("(eg, Dos_emin, Dos_emax, Dos_N): (%f, %f, %f, %d)\n",eg,Dos_emin, Dos_emax, Dos_N);
	    x = (eg-Dos_emin)/(Dos_emax-Dos_emin)*(Dos_N-1);
	    iecenter = (int)x ; 
      //printf("iecenter: %d\n", iecenter);
		...
		*/
          //if (klengthdiff1<range ||klengthdiff2<range ){
            //printf("RELEVANT!\n");
		       ...
		       int MaxL, int **Spe_Num_CBasis, int **Spe_Num_Relation, double range)
{
  ...
  *****************************************************************/
  ...
  double dossum[2][knum_i][knum_j],MulP[5],dE;
  ...
  printf("PDOS routine caption by DH\n");
  ...
  N_Dos=6; i_Dos[0]=SpinP_switch+1; 
      ...
      for (i1=0; i1<=(tnoA-1); i1++){
        ...
        for (ie=0;ie<Dos_N;ie++) {
          for (i=0; i<=(knum_i-1); i++){ // for k-resolution
            for (j=0; j<=(knum_j-1); j++){  
              //printf("Check point\n");
              //printf("%d %d %d %d %d %d\n",spin,GA_AN,i1,ie,i,j);
          Dos[spin][GA_AN][i1][ie][i][j]=0.0;
          //printf("Check point\n");
            } //j
          } //i
        }
  ...
  *******************************************************************/
	...
	for (k=0; k<=(knum_k-1); k++){
	  for (ieg=0;ieg<neg; ieg++) {
		...
		if ( 0<=iemin && iemin<Dos_N && 0<=iemax && iemax <Dos_N ) {
          ...
          //if (klengthdiff1<range ||klengthdiff2<range ){
            //printf("RELEVANT!\n");
		    Dos[spin][GA_AN][i1][ie][i][j] += factor*rval* exp(-xa*xa)/(Dos_Gaussian*pi2*eV2Hartree);
          //}
		  }
		} /* if */
	      } /* i1 */
  ...
  for (iatom=0;iatom<pdos_n;iatom++) {
    ...
    for (L=0;L<=MaxL; L++) {
	  ...
	  if ( (fp_Dos=fopen(file_Dos,"w")) ==NULL )  {
                  ...
                  for (ie=1;ie<q;ie++) {
                    ...
                    for (i1=0;i1< tnoA; i1++) {
                      if (LM== Spe_Num_Relation[wanA][i1]) {
                        for (i=0; i<=(knum_i-1); i++){ // for k-resolution
                          for (j=0; j<=(knum_j-1); j++){ 
                        Doss[spin][ie]+=  Dos[spin][GA_AN][i1][ie][i][j];
                          }//j
                        }//i
                      } 
                    }
	    ...
	    for (ie=0;ie<Dos_N;ie++) {
	      for (spin=0;spin<=SpinP_switch;spin++) dossum[spin][i][j]=0.0;
	      for (i1=0;i1< tnoA; i1++) {
		if (LM== Spe_Num_Relation[wanA][i1]) {
		  for (spin=0;spin<=SpinP_switch;spin++) {
        for (i=0; i<=(knum_i-1); i++){ // for k-resolution
          for (j=0; j<=(knum_j-1); j++){  
		    dossum[spin][i][j]+=  Dos[spin][GA_AN][i1][ie][i][j];
          }//j
        }//i
                  }
		}
  ...
  for (j=0; j<=(knum_j-1); j++){ 
	...
	*/
    ...
    fprintf(fp_Dos,"%lf %lf ", (DosE[ie])*eV2Hartree,  ssum[0][ie]*2.0);
    for (i=0; i<=(knum_i-1); i++){
      for (j=0; j<=(knum_j-1); j++){
        fprintf(fp_Dos,"%lf ", dossum[0][i][j]);
      }//j
    }//i
        fprintf(fp_Dos," \n");
     ...
     )
{
  ...
  if (*method==2) {  //Gaussian
    ...
    printf("K-resolution (CODE by DH Kiem)\n");
    //printf("Please input a value of range near K point in Brillouin zone(double) (CODE by DH Kiem)\n");
    //fgets(buf,1000,stdin); sscanf(buf,"%lf",range);  
  }
